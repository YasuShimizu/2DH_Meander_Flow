...
@jit
def u_cal1(un,gux,guy,u,v_up,fn,gxn,gyn,nx,ny,dsi,dn,dt):
    for i in np.arange(1,nx):
        for j in np.arange(1,ny+1):
            dx=dsi[i,j]; dy=dn[i,j]
    ...
    return fn,gxn,gyn
@jit
def u_cal2(fn,gxn,gyn,u,v_up,un,gux,guy,nx,ny,dsi,dn,dt):
    un=fn.copy()   #これは上手く行く
    gux=gxn.copy()
    guy=gyn.copy()
    for i in np.arange(1,nx):
    ...
    return un,gux,guy
@jit
def v_cal1(vn,gvx,gvy,u_vp,v,fn,gxn,gyn,nx,ny,ds,dnj,dt):
    for i in np.arange(1,nx+1):
        for j in np.arange(1,ny):
            dx=ds[i,j];dy=dnj[i,j]
    ...
    return fn,gxn,gyn
@jit
def v_cal2(fn,gxn,gyn,u_vp,v,vn,gvx,gvy,nx,ny,ds,dnj,dt): 
    vn=fn.copy()
    gvx=gxn.copy()
    gvy=gyn.copy()
    for i in np.arange(1,nx+1):
